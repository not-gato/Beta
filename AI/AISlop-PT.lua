...
local function createUI()
    ...
    createToggle("1. Ler Chat Global", "ChatLogs")
    createToggle("2. Histórico de Conversa", "History")
    createToggle("3. Info Jogadores Detalhada", "PlayerInfo")
    createToggle("4. Info Jogo/Tempo", "GameInfo")
    createToggle("5. Permitir Executar Código", "CodeExec")
    createToggle("6. Info do Executor", "ExecutorInfo")
    createToggle("7. Permitir Todos Falar com IA", "PublicAI")

    local InstructionLabel = Instance.new("TextLabel")
    InstructionLabel.Text = "Instruções Personalizadas (Prompt):"
    ...
    CustomBox.PlaceholderText = "Ex: Seja sarcástico, fale como um pirata..."
    ...
    ResetBtn.Text = "⚠️ REINICIAR SISTEMA (Destravar)"
    ...
    ResetBtn.MouseButton1Click:Connect(function()
        _G.IsGeminiThinking = false
        _G.GeminiHistory = {}
        StarterGui:SetCore("SendNotification", {Title="Sistema", Text="IA Reiniciada com Sucesso."})
    end)
...
local function getDeepPlayerInfo()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        local role = (p == lp) and "VOCÊ (Solicitante)" or "Outro Jogador"
        
        local dist = "Longe/Morto"
        local posStr = "Desconhecido"
        local hp = "N/A"
        local speed = "N/A"
        local jump = "N/A"
        ...
        local verif = p.HasVerifiedBadge and "SIM" or "NÃO"
...
local function getChatHistoryBlock()
    if not Settings.ChatLogs then return "Histórico de Chat: DESLIGADO" end
    ...
    task.spawn(function()
        local success, err = pcall(function()
            ...
            local senderContext = string.format(
                "PERGUNTA DE: %s (@%s). Se perguntarem 'quem sou eu', procure os dados na lista.",
                sender.DisplayName, sender.Name
            )

            local systemPrompt = string.format([[
                Você é uma Assistente IA dentro do Roblox. Responda curto (max 150 chars). Fale Português.
                
                === INSTRUÇÕES PERSONALIZADAS ===
                %s
                
                === TEMPO E INFO DO JOGO ===
                %s
                ...
                REGRAS:
                1. Se pedirem "tp para alguém", gere código usando a Posição (Pos).
                2. Se pedirem "matar", gere código de Kill Aura ou Tool Kill.
                3. Código deve estar em ```lua.
            ...
            if response.StatusCode ~= 200 then
                chat(":: AI :: Erro na API (" .. response.StatusCode .. ").")
                return
            end
            ...
            if result and result.candidates and result.candidates[1] and result.candidates[1].content then
                ...
                chat(":: AI :: Nada a dizer (Filtro/Erro).")
            end
        end)

        if not success then
            warn(":: AI ERRO INTERNO ::", err)
            chat(":: AI :: Erro Interno (Veja F9).")
        end
        _G.IsGeminiThinking = false
    end)
end
...
local function executeCode(code)
    ...
    if func then pcall(func); StarterGui:SetCore("SendNotification", {Title="Sucesso", Text="Executado."})
    else StarterGui:SetCore("SendNotification", {Title="Erro", Text="Script Inválido."}) end
end
...
_G.AskConfirm = function(code)
    ...
    local Title = Instance.new("TextLabel")
    Title.Text = "IA quer executar código:"
...
local function startBot()
    createUI()
    chat('IA Carregada. Para usar, digite "/ai" seguido da sua pergunta.')
end
...
if hasFileAccess and isfile(FILENAME) then
    ...
    local Title = Instance.new("TextLabel"); Title.Text = "INSIRA A API KEY (GEMINI)"; Title.TextColor3 = Color3.new(1,1,1); Title.Size = UDim2.new(1,0,0,30); Title.BackgroundTransparency = 1; Title.Parent = Frame
    local Box = Instance.new("TextBox"); Box.Size = UDim2.new(0.9, 0, 0, 40); Box.Position = UDim2.new(0.05, 0, 0.4, 0); Box.PlaceholderText = "Cole a Key aqui..."; Box.Text = ""; Box.Parent = Frame
    local Save = Instance.new("TextButton"); Save.Text = "SALVAR E INICIAR"; Save.Size = UDim2.new(1, 0, 0, 30); Save.Position = UDim2.new(0, 0, 0.8, 0); Save.BackgroundColor3 = Color3.fromRGB(50, 150, 255); Save.Parent = Frame
