...
local getExecutorName = (identifyexecutor and identifyexecutor) or (getexecutorname and getexecutorname) or function() return "Generic Executor" end
...
local function createUI()
    ...
    createToggle("1. Read Global Chat", "ChatLogs")
    createToggle("2. Conversation History", "History")
    createToggle("3. Detailed Player Info", "PlayerInfo")
    createToggle("4. Game/Time Info", "GameInfo")
    createToggle("5. Allow Code Execution", "CodeExec")
    createToggle("6. Executor Info", "ExecutorInfo")
    createToggle("7. Let Everyone Talk With AI", "PublicAI")

    local InstructionLabel = Instance.new("TextLabel")
    InstructionLabel.Text = "Custom Instructions (Prompt):"
    ...
    CustomBox.PlaceholderText = "Ex: Be sarcastic, speak like a pirate..."
    ...
    ResetBtn.Text = "⚠️ RESET SYSTEM (Unstuck)"
    ...
    ResetBtn.MouseButton1Click:Connect(function()
        _G.IsGeminiThinking = false
        _G.GeminiHistory = {}
        StarterGui:SetCore("SendNotification", {Title="System", Text="AI Reset Successfully."})
    end)
...
local function getGameInfo()
    ...
    info.Executor = Settings.ExecutorInfo and getExecutorName() or "Hidden"
    pcall(function()
...
local function getDeepPlayerInfo()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        local role = (p == lp) and "YOU (Requester)" or "Other Player"
        ...
        local verif = p.HasVerifiedBadge and "YES" or "NO"
...
local function getChatHistoryBlock()
    if not Settings.ChatLogs then return "Chat History: OFF" end
    ...
    task.spawn(function()
        local success, err = pcall(function()
            ...
            local senderContext = string.format(
                "QUESTION BY: %s (@%s). If they ask 'who am I', look for their data in the list.",
                sender.DisplayName, sender.Name
            )

            local systemPrompt = string.format([[
                You are an AI Assistant inside Roblox. Keep answers short (max 150 chars). Speak English.
                ...
                RULES:
                1. If asked to "tp to someone", generate code using their Position (Pos).
                2. If asked to "kill", generate Kill Aura or Tool Kill code.
                3. Code must be in ```lua.
            ...
            if response.StatusCode ~= 200 then
                chat(":: AI :: API Error (" .. response.StatusCode .. ").")
                return
            end
            ...
            if result and result.candidates and result.candidates[1] and result.candidates[1].content then
                ...
                chat(":: AI :: Nothing to say (Filter/Error).")
            end
        end)

        if not success then
            warn(":: AI INTERNAL ERROR ::", err)
            chat(":: AI :: Internal Error (See F9).")
        end
        _G.IsGeminiThinking = false
    end)
end
...
local function executeCode(code)
    ...
    if func then pcall(func); StarterGui:SetCore("SendNotification", {Title="Success", Text="Executed."})
    else StarterGui:SetCore("SendNotification", {Title="Error", Text="Invalid Script."}) end
end
...
_G.AskConfirm = function(code)
    ...
    local Title = Instance.new("TextLabel")
    Title.Text = "AI wants to run code:"
...
local function startBot()
    createUI()
    chat('AI Loaded. To use, type "/ai" followed by your question.')
end
...
if hasFileAccess and isfile(FILENAME) then
    ...
    local Title = Instance.new("TextLabel"); Title.Text = "ENTER API KEY (GEMINI)"; Title.TextColor3 = Color3.new(1,1,1); Title.Size = UDim2.new(1,0,0,30); Title.BackgroundTransparency = 1; Title.Parent = Frame
    local Box = Instance.new("TextBox"); Box.Size = UDim2.new(0.9, 0, 0, 40); Box.Position = UDim2.new(0.05, 0, 0.4, 0); Box.PlaceholderText = "Paste Key here..."; Box.Text = ""; Box.Parent = Frame
    local Save = Instance.new("TextButton"); Save.Text = "SAVE & START"; Save.Size = UDim2.new(1, 0, 0, 30); Save.Position = UDim2.new(0, 0, 0.8, 0); Save.BackgroundColor3 = Color3.fromRGB(50, 150, 255); Save.Parent = Frame
